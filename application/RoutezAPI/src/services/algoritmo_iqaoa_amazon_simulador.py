...
class AlgoritmoIQAOAAmazonSimulador(AlgoritmoBase):
    """
    ...
    """
    TIPO_ALGORITMO = "IQAOA SIMULADOR" # <--- MUDANÇA IQAOA
    locale = ''

    # --- Painel de Controle ---
    _REPS = 1 # Aumentado, pois o circuito é mais simples
    _OPTIMIZER_MAX_ITER = 50 # Aumentado para permitir uma otimização real
    _OPTIMIZER_METHOD = 'COBYLA' # COBYLA é ótimo para VQAs
    _TOTAL_SHOTS_FINAL = 2_000 # Aumentado para amostragem estatisticamente relevante
    ...
    def _get_sampling_device(self, n_qubits: int) -> Device:
        ...
        if not self._USE_QPU:
            ...
            return qml.device("default.qubit", wires=n_qubits, shots=self._TOTAL_SHOTS_FINAL)
...
# FUNÇÃO FACTORY
# --------------------------------------------------------------------------
def get_algoritmo_IQAOA_AmazonSim() -> AlgoritmoIQAOAAmazonSimulador:
    service = AlgoritmoIQAOAAmazonSimulador()
